...
function formatRangeWithChunks(date1, date2, chunks, separator, isRTL) {
  ...
  //return Title <h2>JANUARY 2016</h2>
...
var DayTableMixin = FC.DayTableMixin = {
	...
	renderHeadDateCellHtml: function(date, colspan) {
		...
		return '' +
			'<th class="fc-day-header ' + (date.day() === moment().day() ? 'curr-day ' : '') + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] + '"' +
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderDates: function(isRigid) {
        ...
        //add open current date on click
        var _calendar = this.view.calendar;
	    $('.gotodate_link').on("click", function() {
	        var date = $(this).data('date');

	        if (date) {
	            var localDate = new Date(date);
	            var utcDate = new Date(localDate.getUTCFullYear(), localDate.getUTCMonth(), localDate.getUTCDate(),
	                localDate.getUTCHours(), localDate.getUTCMinutes(), localDate.getUTCSeconds());

	            _calendar.gotoDate(utcDate);
	            _calendar.changeView('agendaDay');
	        }
	    });
	...
	renderNumberCellHtml: function(date) {
	    ...
	    var isPresent = (date.utc().valueOf() >= moment.utc().startOf('day').valueOf());
	    var btnClass = isPresent ? '' : 'fc-dn-btn-past';

    return '' +
      '<td class="' + classes.join(' ') + '" data-date="' + date.format() + '">' +
        '<div class="calendar-header-td clearfix">' +
          '<div class="fc-dn-label gotodate_link" data-date="' + date + '">' + date.date() + '</div>' +
          '<div class="fc-dn-btn right-buttons-container ' + btnClass + '">' +
            '<md-button class="schedule-btn" ng-click="calendarEv.getSchedule(\''+date.format("YYYY-MM-DD")+'\')" simple-tooltip="Schedule" aria-label="..." ng-class="{\'visible\':calendarEv.scheduleAvailable(\'' + date.format("M/D/YYYY") +'\')}">' +
                '<img src="assets/images/default/appoitment/ic_bt_usr_blue.svg">' +
            '</md-button>' +
            '<a href="javascript:void(0)" class="add-new-appointment-btn" ng-click="calendarEv.newAppointment(\'' + date.format("YYYY-MM-DD") + '\')" simple-tooltip="New appointment" aria-label="..."></a>' +
          '</div>' +
        '</div>'+
      '</td>';
	},
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
        ...
        var icon = event.icon ? "<img style='height:10px;width:10px;margin-top:-4px;margin-right:2px;' src='" + event.icon + "' />" : "";
        // Only display a timed events time if it is the starting segment
		if (seg.isStart) {
			timeText = this.getEventTimeText(event);
			if (timeText) {
				timeHtml = '<span class="fc-time">' + icon + htmlEscape(timeText) + '</span>';
			}
		}
				...
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					(isLabeled ?
						'<span>' + // for matchCellWidths
							htmlEscape(slotDate.format(this.labelFormat)).replace(/^([0-9]{1,2})(.*?)$/, '$1 $2') +
						'</span>' :
						''
						) +
				'</td>';
...
var View = FC.View = Class.extend({
	...
	setScroll: function (scrollState) {
	    if (this.scrollerEl) {
	        if (scrollState && scrollState >250) {
	            return this.scrollerEl.scrollTop(scrollState); // operates on scrollerEl by default
	        } else {
	            return this.scrollerEl.scrollTop(250); // avoid scrolling top by default, scroll to 06:00 AM
	        }
	    }
	},
...
function Calendar_constructor(element, overrides) {
    ...
    //change select with views
    $('#calendarViewSelector').val(viewType);
